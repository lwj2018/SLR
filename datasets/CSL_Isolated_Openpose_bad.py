...
class CSL_Isolated_Openpose_bad(data.Dataset):
    
    def __init__(self, skeleton_root, list_file, length=32, is_normalize=True, repeat_time=20):
    ...
    def _load_data(self, path):
        ...
        for i,file in enumerate(file_list):
            ...
            if i>0:
                ...
                # Add bad start and end
                if i==1 or i==len(file_list)-1:
                    for j in range(self.repeat_time): mat.append(mat_i)
        mat = np.array(mat)
        end = time.time()
